...
import {
  BarChart, Search, FileDownload, Refresh, Close,
  Bolt,
} from '@mui/icons-material';
import { useQuery } from '@tanstack/react-query';
import { fetchVolume5DShockers, ShockerStock } from '../services/api';

export default function Volume5DShockersPage() {
  ...
  const [sortField, setSortField] = useState<keyof ShockerStock>('ratio_5d');
  const [sortAsc, setSortAsc] = useState(false);

  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['volume-5d-shockers', filterClass],
    queryFn: () => fetchVolume5DShockers({ page: 1, limit: 100, classification: filterClass === 'ALL' ? undefined : filterClass }),
  ...
  const getClassColor = (cls?: string) => {
    switch (cls) {
      ...
      case 'Moderate': return { bg: '#ab47bc', text: '#fff' };
      default: return { bg: 'action.hover', text: 'text.primary' };
    }
  };

  const handleExportCSV = () => {
    const headers = ['Rank', 'Stock', 'Company', 'Current Price', "Today's Volume", '5D Avg Volume', '5D Volume Ratio', 'Classification', "Today's Change %", 'Buyer %', 'Delivery %', 'Score', 'Signal'];
    ...
    link.setAttribute('download', `nse_5d_volume_shockers_${Date.now()}.csv`);
    ...
    <Box sx={{ pb: 6 }}>
      ...
      <Paper
        ...
        sx={{
          ...
          borderRadius: 3,
          background: isDark
            ? 'linear-gradient(135deg, rgba(171,71,188,0.12) 0%, rgba(106,27,154,0.06) 100%)'
            : 'linear-gradient(135deg, #f3e5f5 0%, #ede7f6 100%)',
          border: '1px solid',
          borderColor: isDark ? 'rgba(171,71,188,0.3)' : 'rgba(171,71,188,0.3)',
          display: 'flex',
      ...
      >
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            <Box
              sx={{
                ...
                bgcolor: '#ab47bc', color: '#fff',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: '0 0 16px rgba(171,71,188,0.5)',
              }}
            >
            ...
            <Box>
              ...
              <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 700 }}>
                5D Volume Ratio = Today's Volume / 5D Average Volume • Weekly Cumulative Volume Expansion
              </Typography>
            </Box>
          </Stack>
        </Box>
        ...
        <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
          <Chip
            icon={<Bolt sx={{ fontSize: '16px !important', color: '#ce93d8' }} />}
            label="5-DAY EXPANSION"
            size="small"
            sx={{ fontWeight: 800, bgcolor: isDark ? 'rgba(171,71,188,0.15)' : '#f3e5f5', color: '#ab47bc' }}
          />
          <Button
            variant="outlined"
            size="small"
      ...
      {/* ── Top 10 5D Volume Shockers Leaderboard ── */}
      <Box sx={{ mb: 3 }}>
            ...
            return (
              <Card
                ...
                sx={{
                  ...
                  borderRadius: 2.5,
                  border: '1px solid',
                  borderColor: idx === 0 ? '#ab47bc' : 'divider',
                  background: isDark
                    ? idx === 0 ? 'linear-gradient(135deg, rgba(171,71,188,0.18) 0%, rgba(11,17,32,0.9) 100%)' : 'rgba(11,17,32,0.6)'
                    : idx === 0 ? '#f3e5f5' : '#ffffff',
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap">
            {['ALL', 'Extreme', 'Very Strong', 'Strong', 'Moderate'].map(cls => (
              <Chip
                ...
                color={filterClass === cls ? 'secondary' : 'default'}
      ...
      {/* ── Master 5D Volume Shockers Table ── */}
      <TableContainer
        component={Paper}
        elevation={0}
        sx={{
        ...
        <Table size="small">
          <TableHead sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
            <TableRow>
              ...
              <TableCell align="right" sx={{ fontWeight: 800, fontSize: 11, cursor: 'pointer', color: '#ab47bc' }} onClick={() => handleSort('ratio_5d')}>
                5D VOLUME SHOCKER {sortField === 'ratio_5d' && (sortAsc ? '▲' : '▼')}
              </TableCell>
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right" sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary' }}>
                    {st.avg_volume_5d?.toLocaleString('en-IN')}
                  </TableCell>
      ...
      >
        {selectedStock && (
          <Box>
            ...
            <Paper elevation={0} sx={{ p: 2, mb: 2.5, borderRadius: 2, bgcolor: isDark ? 'rgba(171,71,188,0.08)' : '#f3e5f5', border: '1px solid', borderColor: 'divider' }}>
              <Typography variant="overline" color="text.secondary" fontWeight={800}>
                5D VOLUME SHOCKER SUMMARY
              </Typography>
              <Stack direction="row" justifyContent="space-between" alignItems="baseline" mt={0.5}>
                <Typography variant="h5" fontWeight={900} sx={{ color: '#ab47bc' }}>
                  5D Volume Shocker: {selectedStock.ratio_5d}x
                </Typography>
                <Chip
              ...
              <Typography variant="body2" sx={{ mt: 1, fontSize: 12, color: 'text.secondary' }}>
                Today's Volume: {selectedStock.today_volume?.toLocaleString('en-IN')} vs 5D Avg: {selectedStock.avg_volume_5d?.toLocaleString('en-IN')}
              </Typography>
            </Paper>
            ...
            >
              Open Technical Chart & OI Breakdown
            </Button>
          </Box>
        )}
      </Drawer>
