...
import {
  Analytics, Search, FileDownload, Refresh, Close,
  Bolt,
} from '@mui/icons-material';
import { useQuery } from '@tanstack/react-query';
import { fetchVolume7DShockers, ShockerStock } from '../services/api';

export default function Volume7DShockersPage() {
  ...
  const [sortField, setSortField] = useState<keyof ShockerStock>('ratio_7d');
  const [sortAsc, setSortAsc] = useState(false);

  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['volume-7d-shockers', filterClass],
    queryFn: () => fetchVolume7DShockers({ page: 1, limit: 100, classification: filterClass === 'ALL' ? undefined : filterClass }),
  ...
  const handleExportCSV = () => {
    const headers = ['Rank', 'Stock', 'Company', 'Current Price', "Today's Volume", '7D Avg Volume', '7D Volume Ratio', 'Classification', "Today's Change %", 'Buyer %', 'Delivery %', 'Score', 'Signal'];
    ...
    link.setAttribute('download', `nse_7d_volume_shockers_${Date.now()}.csv`);
    ...
    <Box sx={{ pb: 6 }}>
      ...
      <Paper
        ...
        sx={{
          ...
          borderRadius: 3,
          background: isDark
            ? 'linear-gradient(135deg, rgba(0,200,83,0.12) 0%, rgba(0,176,255,0.06) 100%)'
            : 'linear-gradient(135deg, #e8f5e9 0%, #e0f7fa 100%)',
          border: '1px solid',
          borderColor: isDark ? 'rgba(0,200,83,0.3)' : 'rgba(0,200,83,0.3)',
          display: 'flex',
      ...
      >
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            <Box
              sx={{
                ...
                bgcolor: '#00c853', color: '#000',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: '0 0 16px rgba(0,200,83,0.5)',
              }}
            >
              <Analytics sx={{ fontSize: 26 }} />
            </Box>
            <Box>
              ...
              <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 700 }}>
                7D Volume Ratio = Today's Volume / 7D Average Volume • Multi-Session Accumulation Shocker
              </Typography>
            </Box>
          </Stack>
        </Box>
        ...
        <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
          <Chip
            icon={<Bolt sx={{ fontSize: '16px !important', color: '#00c853' }} />}
            label="7-SESSION BREAKOUT"
            size="small"
            sx={{ fontWeight: 800, bgcolor: isDark ? 'rgba(0,200,83,0.15)' : '#e8f5e9', color: '#00c853' }}
          />
          <Button
            variant="outlined"
            size="small"
            ...
            return (
              <Card
                ...
                sx={{
                  ...
                  borderRadius: 2.5,
                  border: '1px solid',
                  borderColor: idx === 0 ? '#00c853' : 'divider',
                  background: isDark
                    ? idx === 0 ? 'linear-gradient(135deg, rgba(0,200,83,0.18) 0%, rgba(11,17,32,0.9) 100%)' : 'rgba(11,17,32,0.6)'
                    : idx === 0 ? '#e8f5e9' : '#ffffff',
      ...
      {/* ── Master 7D Volume Shockers Table ── */}
      <TableContainer
        component={Paper}
        elevation={0}
        sx={{
        ...
        <Table size="small">
          <TableHead sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
            <TableRow>
              ...
              <TableCell align="right" sx={{ fontWeight: 800, fontSize: 11, cursor: 'pointer', color: '#00c853' }} onClick={() => handleSort('ratio_7d')}>
                7D VOLUME SHOCKER {sortField === 'ratio_7d' && (sortAsc ? '▲' : '▼')}
              </TableCell>
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right" sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary' }}>
                    {st.avg_volume_7d?.toLocaleString('en-IN')}
                  </TableCell>
      ...
      >
        {selectedStock && (
          <Box>
            ...
            <Paper elevation={0} sx={{ p: 2, mb: 2.5, borderRadius: 2, bgcolor: isDark ? 'rgba(0,200,83,0.08)' : '#e8f5e9', border: '1px solid', borderColor: 'divider' }}>
              <Typography variant="overline" color="text.secondary" fontWeight={800}>
                7D VOLUME SHOCKER SUMMARY
              </Typography>
              <Stack direction="row" justifyContent="space-between" alignItems="baseline" mt={0.5}>
                <Typography variant="h5" fontWeight={900} color="success.main">
                  7D Volume Shocker: {selectedStock.ratio_7d}x
                </Typography>
                <Chip
              ...
              <Typography variant="body2" sx={{ mt: 1, fontSize: 12, color: 'text.secondary' }}>
                Today's Volume: {selectedStock.today_volume?.toLocaleString('en-IN')} vs 7D Avg: {selectedStock.avg_volume_7d?.toLocaleString('en-IN')}
              </Typography>
            </Paper>
